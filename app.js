...
window.checkoutWhatsApp = function() {
  if (cartItems.length === 0) {
    alert('Your cart is empty! Please add dishes to order.');
    return;
  }

  let text = `*New Takeaway Order - The Silver Spoon*\n`;
  text += `-----------------------------------\n`;
  ...
  text += `Please confirm my takeaway order!`;

  const encoded = encodeURIComponent(text);
  const whatsappUrl = `https://wa.me/919934318146?text=${encoded}`;
